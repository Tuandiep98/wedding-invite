...
- **Query form:** `?e={eventId}&k={guestCode}`. Works on any host. On GitHub Pages, `404.html` converts pretty links into the query form, but that response is HTTP 404 with no `og:*`, so Zalo/Messenger show no preview. `links.js` therefore forces the query form when the base URL is `*.github.io`.
