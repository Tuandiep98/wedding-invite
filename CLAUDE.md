...
- **Site:** `https://tuandiepthuthao.date/` (GitHub Pages + custom domain, domain registered at Cloudflare). `WEDDING_SITE_URL` in `js/events.js`; `links.html` always builds links on it.
- **Short form (sent to guests):** `?{guestCode}`, e.g. `/?phu`. The event is inferred from the guest (`event` or the list's `defaultEvent`), which is why `js/guests/*.js` load synchronously in `<head>` **before** `events.js`. Extra params appended by Zalo/Facebook (`fbclid=…`) are ignored.
...
- **Query form:** `?e={eventId}&k={guestCode}`. Works on any host; `links.html` uses it only when a guest gets a variant other than their default. On GitHub Pages, `404.html` converts pretty links (`/nha-trai/phu` → `?e=nha-trai&k=phu`, `/phu` → `?phu`) with JS, but that response is HTTP 404 with no `og:*`, so Zalo/Messenger show no preview. Always send the short or query form.
...
- `js/guests/nha-trai.js`, `js/guests/nha-gai.js`: guest lists pushed into `window.WEDDING_GUESTS`. Loaded **synchronously** in `<head>` before `events.js` (short links need them to pick the event).
...
- `links.html` + `js/links.js`: **internal page for the couple** (`noindex`, not linked from the site). Features:
  ...
  - "Đã gửi" checkboxes stored in `localStorage` (per origin). On `*.github.io` a button copies a `links.html#sent=<side>:<code>,…` link that re-imports them on the new domain.
...
| Link preview + favicon                                | `assets/og-image.jpg` (1200×630, shared by every variant since crawlers don't run JS), `og:*` meta in `<head>`; `og:image` is absolute (`https://tuandiepthuthao.date/…`), update it if the domain changes. `favicon.ico`, `assets/favicon-32.png`, `assets/apple-touch-icon.png` (囍 gold on dark red), `theme-color`                                                                                                                                                                                                                                                                              |
